...
public class DW_CameraFade : MonoBehaviour {
    private static DW_CameraFade mInstance = null;

    private static DW_CameraFade instance {
        get {
            if (mInstance == null) {
                mInstance = GameObject.FindObjectOfType(typeof(DW_CameraFade)) as DW_CameraFade;

                if (mInstance == null) {
                    mInstance = new GameObject("DW_CameraFade").AddComponent<DW_CameraFade>();
                }
            }
    ...
    void Awake() {
        if (mInstance == null) {
            mInstance = this as DW_CameraFade;
            instance.init();
        }
    }
